...
public class FrmManteProd extends JFrame {
	...
	public FrmManteProd() {
		...
		cboProveedores.setBounds(300, 104, 110, 22);
		contentPane.add(cboProveedores);
		
		JLabel lblEstado = new JLabel("Estado:");
		lblEstado.setBounds(230, 136, 102, 14);
		contentPane.add(lblEstado);

		cboEstado = new JComboBox();
		cboEstado.setBounds(300, 134, 110, 22);
	...
	void registrar() {
		...
		String nomProd = txtDescripcion.getText();
		int idCat = cboCategorias.getSelectedIndex();
		int stkProd = Integer.parseInt(txtStock.getText());
		double preProd = Double.parseDouble(txtPrecio.getText());
		int idPrv = cboProveedores.getSelectedIndex();
		int estProd = cboEstado.getSelectedIndex();
		
		// Objeto a grabar
		Producto p = new Producto();
		p.setId_prod(idProd);
		p.setDes_prod(nomProd);
		p.setIdcategoria(idCat);
		p.setStk_prod(stkProd);
		p.setPre_prod(preProd);
		p.setIdproveedor(idPrv);
		p.setEst_prod(estProd);
	...
	void llenaCombo() {
		...
		// select * from tb_xxxx
		sql = "select pv from Proveedor pv";
